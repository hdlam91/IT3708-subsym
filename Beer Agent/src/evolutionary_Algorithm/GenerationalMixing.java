...
public class GenerationalMixing<T> extends AdultSelection<T>{
	...
	public Population<T> findAdultsFromPopulations(Population<T> pop1, Population<T> pop2, List<Double> pheno1Values, List<Double> pheno2Values, int sizeOfOutComePopolation) {
		List<Individual<T>> individuals1 = pop1.getIndividuals();
		...
		for (int i = 0; i < individuals1.size(); i++) {
			if(!hm.containsKey(pheno1Values.get(i)))
				hm.put(pheno1Values.get(i), new ArrayList<Individual<T>>());
			hm.get(pheno1Values.get(i)).add(individuals1.get(i));
		}
