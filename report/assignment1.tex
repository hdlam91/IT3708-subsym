...
The bots doesn't always accomplish the mission either, they tend to get stuck now and then. This usually happens when there's two or three bots on both opposite side of the box. Both "team" think they are pushing the box, and they trust their neighbours - thus believing that they are actually pushing the box forward when they in reality don't. 
In the start we decided to try to find the box by using the camera (because we thought that the food source is the only object that is red), however there's red colors in the grey floor and on the led of the e-puck bots too. The camera approach didn't work as well as we had hoped it would, so we decided to scrap it.

The bots could also be trapped either inside a swarm of e-pucks or between the box and the wall. Because they only have three behaviours established they don't know if the box has reached its location or not. This makes it so the bots runs for ever even though they've accomplished the task.
